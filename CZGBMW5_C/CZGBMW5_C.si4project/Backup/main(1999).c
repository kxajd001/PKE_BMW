...
void GPIO_Configuration(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_PIN_NXP_LOCK | GPIO_PIN_NXP_OPEN;  //NXP_LOCK/OPEN
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_OD;  //开漏输出
	GPIO_Init(GPIO_NXP_LOCK, &GPIO_InitStructure);
	GPIO_SetBits(GPIO_NXP_LOCK, GPIO_PIN_NXP_LOCK);
	GPIO_SetBits(GPIO_NXP_OPEN, GPIO_PIN_NXP_OPEN);
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_PIN_125K_L| GPIO_PIN_125K_R; //低频发射
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP; //复用推挽输出    
...
void SetGpioPinNxpOpen(uint8_t PinNxpOpenState)
{
	GPIO_InitTypeDef  GPIO_InitStructure; 
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;  
	GPIO_InitStructure.GPIO_Pin = GPIO_PIN_NXP_OPEN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_OD;
	GPIO_Init(GPIO_NXP_OPEN, &GPIO_InitStructure);
	if(PinNxpOpenState != Bit_SET) 
	{
		GPIO_ResetBits(GPIO_NXP_OPEN, GPIO_PIN_NXP_OPEN);
	}
	else 
	{
		GPIO_SetBits(GPIO_NXP_OPEN, GPIO_PIN_NXP_OPEN);
...
{
	//SystemInit();
	NVIC_SetVectorTable(NVIC_VectTab_FLASH, 0x04000);     //中断向量重置
	RCC_Configuration();
	GPIO_Configuration();
	RTC_Configuration(); 
